...
elseif (isset($_GET['abo'])){
    ...
    $abo = htmlspecialchars($_GET['abo']);

    $abonnement = $bdd->prepare('SELECT * FROM type_abonnement WHERE id = :id');
    $abonnement->execute(array('id' => $abo));

    $abonnement = $abonnement->fetch(PDO::FETCH_ASSOC);

    if ($abonnement){

        $sku = '0000'.$abonnement['id'].strtoupper(substr($abonnement['nom'], 0, 3));
        $sku = str_replace(' ', 0, $sku);
        ...
        $payment_data = [
            ...
            "transactions" => [
                [
                    ...
                    "description" => "Paiement de votre abonnement ".$abonnement['nom']." sur le site Dontatune"
                ]
            ]
        ];
        ...
        $msg = 'Votre paiement n\' a pas pu être pris en compte, réessayez ultérieurement';
    }
}
